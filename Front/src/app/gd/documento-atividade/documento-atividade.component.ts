...
import { Documento_atividade } from "../model/documento-atividade";
...
export class DocumentoAtividadeComponent implements OnInit {
  ...
  documento_atividades: any;
  documento_atividade = new Documento_atividade();
  documento_atividade_edit = new Documento_atividade();
  ...
  ngOnInit() {
    ...
    this.breadCrumbItems = [
      { label: "Roval" },
      { label: "Gerir Documento-Atividade", active: true },
    ];
  ...
  carregarLista() {
    ...
    this.generalService
      ...
      .subscribe((res: any) => {
        if (res.data) {
          console.log("LISTA DE DOCUMENTO ATIVIDADES", res.data.data);
          this.documento_atividades = res.data.data;
  ...
  save() {
    ...
    if (this.momentForm.invalid) {
      console.log("Dados inválidos", this.documento_atividade);
      console.log("ERROS", this.momentForm.errors);
      return;
    }

    console.log("DADOS válidos", this.documento_atividade);
    this.generalService
      .execute(
        "gd-documento-actividade",
        GeneralConstants.CRUD_OPERATIONS.INSERT,
        this.documento_atividade
      )
      .subscribe((res: any) => {
        if (res.data) {
          this.carregarLista();
  ...
  removeItem() {
    if (this.recoverOrRemoveItem === "recover") {
      this.documento_atividade.estado = 1;
    }

    if (this.recoverOrRemoveItem === "remove") {
      this.documento_atividade.estado = 0;
    }
    this.save();
    console.log("deletar este item", this.documento_atividade);
    this.removeItemModal?.hide();
  }

  editUser(document: any, edit: boolean) {
    this.momentForm.get("estado").disable();
    this.documento_atividade = { ...document };
    this.editar = edit;
  ...
  resetForm() {
    this.documento_atividade = new Documento_atividade(); // Reinstancia o modelo
