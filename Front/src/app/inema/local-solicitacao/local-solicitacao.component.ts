...
import { LocalSolicitacao } from "../model/local_solicitacao";
...
@Component({
  selector: "app-local-solicitacao",
  templateUrl: "./local-solicitacao.component.html",
  styleUrls: ["./local-solicitacao.component.css"],
})
export class LocalSolicitacaoComponent {
  ...
  localSolicitacoes: any;
  localSolicitacao = new LocalSolicitacao();
  ...
  carregarLista() {
    ...
    this.generalService
      ...
      .subscribe((res: any) => {
        ...
        if (res.data) {
          console.log(res);
          this.localSolicitacoes = res.data.data;
  ...
  save() {
    if (this.momentForm.invalid) {
      console.log("Dados inválidos", this.localSolicitacao);
      console.log("ERROS", this.momentForm.errors);
      return;
    }
    console.log("DADOS válidos", this.localSolicitacao);

    this.generalService
      .execute(
        "ocorrencias-local-solicitacoes",
        GeneralConstants.CRUD_OPERATIONS.INSERT,
        this.localSolicitacao
      )
      .subscribe((res: any) => {
        ...
        if (res.data) {
          ...
          this.carregarLista();
          this.localSolicitacao = new LocalSolicitacao();
